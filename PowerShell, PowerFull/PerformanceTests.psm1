...
function Test-MemoryImpact {
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory=$true)]
        [scriptblock]$ScriptBlock
    )
    #$StartMemory = [GC]::GetTotalAllocatedBytes()
    $StartMemory = (Get-Process -Id $PID).WorkingSet
    Invoke-Command -ScriptBlock $ScriptBlock
    #$EndMemory = [GC]::GetTotalAllocatedBytes()
    $EndMemory = (Get-Process -Id $PID).WorkingSet
    Write-Output ("`e[94mExection consumed `e[91m{0:N}`e[94m KB of memory`e[0m" -f $(($EndMemory - $StartMemory) / 1KB))
}

Export-ModuleMember -Function Test-MemoryImpact
